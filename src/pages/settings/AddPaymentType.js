...
const AddPaymentType = () => {
  ...
  const paymentTypeRegistration = async (e) => {
    ...
    if (!name) {
      ...
      try {
        ...
        const dataRef = doc(collection(db, "paymentTypes"));
        await setDoc(dataRef, {
          ...
          .then(() => {
            ...
            setDescription("");
            getTypes();
            toast.success("Payment type is saved successfully");
            setLoading(false);
          })
          .catch((error) => {
  ...
  return (
    <div>
      ...
      >
        <Add className="mt-2 py-0.5" /> <p className="py-2">Create Payment Type</p>
      </div>
      ...
      >
        <Box sx={style} className="rounded-md">
          <div>
            <h3 className="text-center text-xl py-4">Add Payment Type</h3>
            <div>
