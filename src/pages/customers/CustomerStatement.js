...
const CustomerStatement = ({customerID}) => {
  ...
  const generatePDF = () => {
    ...
    const columnTwoData = [
      `TZS ${formatter.format(reportData?.openingBalance)}`,
      `TZS ${formatter.format(reportData?.fuelTaken)}`,
      `TZS ${formatter.format(reportData?.totalPayment)}`,
      `TZS ${formatter.format(reportData?.closingBalance)}`,
    ];
    for (let i = 2; i <= 5; i++) {
