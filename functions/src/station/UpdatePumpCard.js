...
exports.createPumpCard = onCall(async (request) => {
    try {
        ...
        .collection(day)
        .doc(pumpID)
        .update({ newCardOM: 0, newCardCM, oldCardCM, cardSwap: true });

        return { status: 200, message: "Pump card is updated successfully" };
    } catch (error) {
        console.error("Error updating pump card:", error);
        return { status: 500, message: "Failed to update pump card" };
  }
});
