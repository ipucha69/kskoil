...
exports.dailySupplierFinancialStatements = onCall(async (request) => {
    try {
        ...
        const filterAndFormatTransactions = (transactions, type, startDate, endDate) => {
            return transactions.flatMap((transaction) => {
                const seconds = transaction.date.seconds || transaction.date._seconds;
                const day = moment.unix(seconds).format("DD-MM-YYYY");
                if (moment(day, "DD-MM-YYYY").isBetween(moment(startDate, "YYYY-MM-DD"), moment(endDate, "YYYY-MM-DD"), null, '[]')) {
        ...
        const filteredPurchases = filterAndFormatTransactions(supplierPurchases, "expense", startDate, endDate);
        const filteredPayments = filterAndFormatTransactions(supplierPayments, "payment", startDate, endDate);
        ...
        return {
            ...
            data: {
                ...
                title: `Financial statement as of ${formatMonthYear(startDate)} to ${formatMonthYear(endDate)}`
            },
        };
    } catch (error) {
